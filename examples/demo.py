"""
Comprehensive demo of the rate limiting system using the celery-throttle library.
...
def demo_basic_functionality():
    ...
    fast_queue = throttle.create_queue("10/10s")  # 10 tasks per 10 seconds
    print(f"   Fast queue: {fast_queue}")

    slow_queue = throttle.create_queue("2/60s")   # 2 tasks per 60 seconds
    print(f"   Slow queue: {slow_queue}")

    very_slow_queue = throttle.create_queue("1/300s")  # 1 task per 5 minutes
    ...
    config = throttle.queue_manager.get_queue_config(fast_queue)
    if config:
    ...
    status = throttle.get_rate_limit_status(fast_queue)
    if status:
    ...
    stats = throttle.get_queue_stats(fast_queue)
    if stats:
    ...
    throttle.update_rate_limit(fast_queue, "5/30s")
    updated_config = throttle.queue_manager.get_queue_config(fast_queue)
    print(f"   Updated rate limit: {updated_config.rate_limit}")

    print(f"\n8. Testing queue deactivation...")
    throttle.deactivate_queue(slow_queue)
...
def demo_rate_limiting_behavior():
    ...
    test_queue = throttle.create_queue("3/10s")  # 3 tasks per 10 seconds
    print(f"Created test queue: {test_queue} (3 tasks per 10 seconds)")

    print(f"\n1. Testing token bucket behavior...")
    for i in range(8):
        can_process, wait_time = throttle.queue_manager.can_process_task(test_queue)
